...
class BartDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        hidden_states = self.fc2(hidden_states)
        hidden_states = F.dropout(hidden_states, p=self.dropout, training=self.training)
        # print(hidden_states.shape,decsion_from_encoder.shape)
        hidden_states = residual + hidden_states + decsion_from_encoder#decisonis merger here
        hidden_states = self.final_layer_norm(hidden_states)
        # print(hidden_states.shape,'hidden_states shape')
        # print(decsion_from_encoder.shape,'decsion_from_encoder')
        # sys.exit(0)
...
class BartModel(BartPretrainedModel):
    def __init__(self, config: BartConfig):
        ...
        # self.pre_classifier = torch.nn.Linear(2304, 768)
        self.pre_decoder = torch.nn.Linear(768, 768)
    ...
    def forward(
        ...
        # print(encoder_outputs1[0].shape,'enoceder shape after this adaptive has (1,768)')
        ...
        # pooled_output=torch.cat((pooled_encoder_outputs1,pooled_encoder_outputs2,pooled_encoder_outputs3),dim=1)
        # pooled_output=torch.flatten(pooled_output).unsqueeze(dim=0)
        ...
        # print(feat_index.shape,'pooled_output')
        # sys.exit(0)
        pooled_output_decode = self.pre_decoder(feat_index)
        pooled_output = F.relu(pooled_output_decode)
        ...
        encoder_outputs=BaseModelOutput(
            ...
            last_hidden_state=pooled_output_decode,
...
class BartForConditionalGeneration(BartPretrainedModel):
    ...
    def forward(
        ...
        decision_output=self.adapt(decision_output)
        # print(type(decision_output),decision_output)
