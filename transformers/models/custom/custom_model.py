...
#         key_value_states1: Optional[torch.Tensor] = None,
#         key_value_states2: Optional[torch.Tensor] = None,
#         key_value_states3: Optional[torch.Tensor] = None,
...
#         # for the decoder
#         is_cross_attention = key_value_states1 is not None and key_value_states2 is not None and key_value_states3 is not None
...
#             key_states1 = past_key_values[0][0]
#             value_states1 = past_key_values[0][1]
#             key_states2 = past_key_values[1][0]
#             value_states2 = past_key_values[1][1]
#             key_states3 = past_key_values[2][0]
#             value_states3 = past_key_values[2][1]
#         elif is_cross_attention:
#             # cross_attentions
#             key_states1 = self._shape(self.k_proj(key_value_states1), -1, bsz)
#             value_states1 = self._shape(self.v_proj(key_value_states1), -1, bsz)
#             key_states2 = self._shape(self.k_proj(key_value_states2), -1, bsz)
#             value_states2 = self._shape(self.v_proj(key_value_states2), -1, bsz)
#             key_states3 = self._shape(self.k_proj(key_value_states3), -1, bsz)
#             value_states3 = self._shape(self.v_proj(key_value_states3), -1, bsz)
...
#                 past_key_values=((key_states1, value_states1),(key_states2, value_states2),(key_states3, value_states3))
#             else:
...
class BartAttention(nn.Module):
    ...
    def forward(
        ...
        proj_shape = (bsz * self.num_heads, -1, self.head_dim)
        query_states = self._shape(query_states, tgt_len, bsz).view(*proj_shape)
        # print(len(key_states),'key_states',type(key_states))
        # print(len(value_states),'value_states',type(value_states))
        key_states = key_states.view(*proj_shape)
        value_states = value_states.view(*proj_shape)
...
class BartDecoderLayer(nn.Module):
    ...
    def forward(
        self,
        #check_this encoder_attention_mask
        #check_this encoder_layer_head_mask
        hidden_states: torch.Tensor,
        attention_mask: Optional[torch.Tensor] = None,
        ...
        if encoder_hidden_states1 is not None and encoder_hidden_states2 is not None and encoder_hidden_states3 is not None:
            residual = hidden_states

            # cross_attn cached key/values tuple is at positions 3,4 of present_key_value tuple
            # l=len(past_key_value) if past_key_value is not None else 0
            # print(l,'past_key_value')
            cross_attn_past_key_value1 = past_key_value[-2:] if past_key_value is not None else None
            hidden_states1, cross_attn_weights1, cross_attn_present_key_value1 = self.encoder_attn(
                ...
                past_key_value=cross_attn_past_key_value1,
                output_attentions=output_attentions,
            )
            cross_attn_past_key_value2 = past_key_value[-2:] if past_key_value is not None else None
            hidden_states2, cross_attn_weights2, cross_attn_present_key_value2 = self.encoder_attn(
                hidden_states=hidden_states,
            ...
            cross_attn_past_key_value3 = past_key_value[-2:] if past_key_value is not None else None
            hidden_states3, cross_attn_weights3, cross_attn_present_key_value3 = self.encoder_attn(
                hidden_states=hidden_states,
                key_value_states=encoder_hidden_states3,
                attention_mask=encoder_attention_mask,
                layer_head_mask=encoder_layer_head_mask,
                past_key_value=cross_attn_past_key_value3,
                output_attentions=output_attentions,
            )

            hidden_states1 = F.dropout(hidden_states1, p=self.dropout, training=self.training)
            ...
            hidden_states = residual + hidden_states1+hidden_states2+hidden_states3
            hidden_states = self.encoder_attn_layer_norm(hidden_states)

            # add cross-attn to positions 3,4 of present_key_value tuple
            present_key_value = present_key_value + (cross_attn_present_key_value1,cross_attn_present_key_value2,cross_attn_present_key_value3,)
        ...
        if output_attentions:
            outputs += (self_attn_weights, cross_attn_weights1,cross_attn_weights2,cross_attn_weights3)
...
class BartDecoder(BartPretrainedModel):
    ...
    def forward(
        ...
        k=len(past_key_values) if past_key_values is not None else 0 
        ...
        #check_this_mask is one
        # expand encoder attention mask
        if encoder_hidden_states1 is not None and encoder_hidden_states2 is not None and encoder_hidden_states3 is not None and encoder_attention_mask is not None:
            # [bsz, seq_len] -> [bsz, 1, tgt_seq_len, src_seq_len]
        ...
        #check_this
        all_cross_attentions = () if (output_attentions and encoder_hidden_states1 is not None and encoder_hidden_states2 is not None and encoder_hidden_states2 is not None) else None
        ...
        for idx, decoder_layer in enumerate(self.layers):
            # print(idx,decoder_layer,'layers_count')
            # add LayerDrop (see https://arxiv.org/abs/1909.11556 for description)
            if output_hidden_states:
...
class BartModel(BartPretrainedModel):
    ...
    def forward(
        ...
        if return_dict and not isinstance(encoder_outputs1, BaseModelOutput):
        ...
        if return_dict and not isinstance(encoder_outputs2, BaseModelOutput):
            encoder_outputs2 = BaseModelOutput(
                last_hidden_state=encoder_outputs2[0],
        ...
        if return_dict and not isinstance(encoder_outputs3, BaseModelOutput):
    ...
    #     hidden_states: torch.Tensor,
    #     attention_mask: Optional[torch.Tensor] = None,
    #     encoder_layer_head_mask1: Optional[torch.Tensor] = None,
    #     layer_head_mask: Optional[torch.Tensor] = None,
    #     past_key_value: Optional[Tuple[torch.Tensor]] = None,
    #     output_attentions: Optional[bool] = False,
    #     encoder_hidden_states1: Optional[torch.Tensor] = None,
    ...
    #     encoder_layer_head_mask2: Optional[torch.Tensor] = None,
    #     encoder_hidden_states3: Optional[torch.Tensor] = None,
    #     encoder_attention_mask3: Optional[torch.Tensor] = None,
    #     encoder_layer_head_mask3: Optional[torch.Tensor] = None,
    #     use_cache: Optional[bool] = True,
    # ):

        if not return_dict:
            return decoder_outputs + encoder_outputs
        #check_this encoder_outputs1
        return Seq2SeqModelOutput(
            ...
            encoder_last_hidden_state=encoder_outputs1.last_hidden_state,
            encoder_hidden_states=encoder_outputs1.hidden_states,
            encoder_attentions=encoder_outputs1.attentions,
        )
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs
        ...
        return {
            ...
            "encoder_outputs3": encoder_outputs,
