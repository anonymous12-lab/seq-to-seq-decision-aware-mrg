...
class PreTrainedTokenizerBase(SpecialTokensMixin):
    ...
    model_input_names: List[str] = ["input_ids", "input_ids2","input_ids3", "token_type_ids", "attention_mask","attention_mask2","attention_mask3"]
    ...
    def pad(
        ...
        # print(self.model_input_names,'self.model_input_names[0]')
        # print(encoded_inputs,'self.model_input_names[0]')
        # sys.exit(0)
        self.model_input_names=['input_ids','input_ids2','input_ids3', 'attention_mask','attention_mask2','attention_mask3']
        ...
        required_input2 = encoded_inputs[self.model_input_names[1]]
        required_input3 = encoded_inputs[self.model_input_names[2]]


        if not required_input:
            if return_attention_mask:
                encoded_inputs["attention_mask"] = []
                encoded_inputs["attention_mask2"] = []
                encoded_inputs["attention_mask3"] = []
        ...
        first_element2 = required_input[0]
        first_element3 = required_input[0]
        ...
        if isinstance(first_element, (list, tuple)):
            ...
            index = 0
            while len(required_input2[index]) == 0:
                index += 1
            if index < len(required_input2):
                first_element2 = required_input2[index][0]
            index = 0
            while len(required_input3[index]) == 0:
                index += 1
            if index < len(required_input3):
                first_element3 = required_input3[index][0]
        ...
        # print(max_length,'max_length3')None
        # print(padding_strategy,'padding_strategy')PaddingStrategy.LONGEST
        ...
        if padding_strategy == PaddingStrategy.LONGEST:
            ...
            padding_strategy = PaddingStrategy.MAX_LENGTH
            max_length2 = max(len(inputs) for inputs in required_input2)
            # padding_strategy2 = PaddingStrategy.MAX_LENGTH
            max_length3 = max(len(inputs) for inputs in required_input3)
            # padding_strategy3 = PaddingStrategy.MAX_LENGTH
        # print(max_length,max_length2,max_length3,pad_to_multiple_of)
        

        batch_outputs = {}
        # print(encoded_inputs,'encoded')
        for i in range(batch_size):
            ...
            for key,value in outputs.items():
                ...
                # if t2[0] not in batch_outputs:
                #     batch_outputs[t2[0]] = []
                # batch_outputs[t2[0]].append(t[1])                
                # if t3[0] not in batch_outputs:
                #     batch_outputs[t3[0]] = []
                # batch_outputs[t3[0]].append(t3[1])
    ...
    def _pad(
        ...
        max_length: Optional[int] = None,
        max_length2: Optional[int] = None,
        max_length3: Optional[int] = None,
        padding_strategy: PaddingStrategy = PaddingStrategy.DO_NOT_PAD,
        pad_to_multiple_of: Optional[int] = None,
        return_attention_mask: Optional[bool] = None,
    ) -> dict:
        ...
        required_input2 = encoded_inputs[self.model_input_names[1]]
        required_input3 = encoded_inputs[self.model_input_names[2]]
        # print(len(required_input),'len required_input',len(required_input2),'len required_input',len(required_input3),'len required_input',max_length,max_length2,max_length3,padding_strategy)


        if padding_strategy == PaddingStrategy.LONGEST:
            max_length = len(required_input)
        if padding_strategy == PaddingStrategy.LONGEST:
            max_length2 = len(required_input2)
        if padding_strategy == PaddingStrategy.LONGEST:
            max_length3 = len(required_input3)



        if max_length is not None and pad_to_multiple_of is not None and (max_length % pad_to_multiple_of != 0):
            # print('does not goes into this')
            max_length = ((max_length // pad_to_multiple_of) + 1) * pad_to_multiple_of
            max_length2 = ((max_length2 // pad_to_multiple_of) + 1) * pad_to_multiple_of
            max_length3 = ((max_length3 // pad_to_multiple_of) + 1) * pad_to_multiple_of

        needs_to_be_padded = padding_strategy != PaddingStrategy.DO_NOT_PAD and len(required_input) != max_length
        needs_to_be_padded2 = padding_strategy != PaddingStrategy.DO_NOT_PAD and len(required_input2) != max_length2
        needs_to_be_padded3 = padding_strategy != PaddingStrategy.DO_NOT_PAD and len(required_input3) != max_length3
        # print(needs_to_be_padded,needs_to_be_padded2,needs_to_be_padded3)
        ...
        if needs_to_be_padded:
            difference2 = max_length2 - len(required_input2)
            if self.padding_side == "right":
                if return_attention_mask:
                    encoded_inputs["attention_mask2"] = [1] * len(required_input2) + [0] * difference2
                if "token_type_ids2" in encoded_inputs:
                    encoded_inputs["token_type_ids2"] = (
                        encoded_inputs["token_type_ids2"] + [self.pad_token_type_id] * difference2
                    )
                if "special_tokens_mask2" in encoded_inputs:
                    encoded_inputs["special_tokens_mask2"] = encoded_inputs["special_tokens_mask2"] + [1] * difference2
                encoded_inputs[self.model_input_names[1]] = required_input2 + [self.pad_token_id] * difference2
            elif self.padding_side == "left":
                if return_attention_mask:
                    encoded_inputs["attention_mask2"] = [0] * difference2 + [1] * len(required_input2)
                if "token_type_ids2" in encoded_inputs:
                    encoded_inputs["token_type_ids2"] = [self.pad_token_type_id] * difference2 + encoded_inputs[
                        "token_type_ids2"
                    ]
                if "special_tokens_mask2" in encoded_inputs:
                    encoded_inputs["special_tokens_mask2"] = [1] * difference2 + encoded_inputs["special_tokens_mask2"]
                encoded_inputs[self.model_input_names[1]] = [self.pad_token_id] * difference2 + required_input2
            else:
                raise ValueError("Invalid padding strategy:" + str(self.padding_side))
        elif return_attention_mask and "attention_mask2" not in encoded_inputs:
            encoded_inputs["attention_mask2"] = [1] * len(required_input2)

        if needs_to_be_padded:
            difference3 = max_length3 - len(required_input3)
            if self.padding_side == "right":
                if return_attention_mask:
                    encoded_inputs["attention_mask3"] = [1] * len(required_input3) + [0] * difference3
                if "token_type_ids3" in encoded_inputs:
                    encoded_inputs["token_type_ids3"] = (
                        encoded_inputs["token_type_ids3"] + [self.pad_token_type_id] * difference3
                    )
                if "special_tokens_mask3" in encoded_inputs:
                    encoded_inputs["special_tokens_mask3"] = encoded_inputs["special_tokens_mask3"] + [1] * difference3
                encoded_inputs[self.model_input_names[2]] = required_input3 + [self.pad_token_id] * difference3
            elif self.padding_side == "left":
                if return_attention_mask:
                    encoded_inputs["attention_mask3"] = [0] * difference3 + [1] * len(required_input3)
                if "token_type_ids3" in encoded_inputs:
                    encoded_inputs["token_type_ids3"] = [self.pad_token_type_id] * difference3 + encoded_inputs[
                        "token_type_ids3"
                    ]
                if "special_tokens_mask3" in encoded_inputs:
                    encoded_inputs["special_tokens_mask3"] = [1] * difference3 + encoded_inputs["special_tokens_mask3"]
                encoded_inputs[self.model_input_names[2]] = [self.pad_token_id] * difference3 + required_input3
            else:
                raise ValueError("Invalid padding strategy:" + str(self.padding_side))
        elif return_attention_mask and "attention_mask3" not in encoded_inputs:
            encoded_inputs["attention_mask3"] = [1] * len(required_input3)
        # print(needs_to_be_padded,needs_to_be_padded2,needs_to_be_padded3,'needs_to_be_padded')
        # print(len(required_input),'len required_input',len(required_input2),'len required_input',len(required_input3),'len required_input',max_length,max_length2,max_length3,padding_strategy)
        return encoded_inputs
