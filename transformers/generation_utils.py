...
class GenerationMixin:
    ...
    def prepare_inputs_for_generation(self, input_ids: torch.LongTensor, input_ids2: torch.LongTensor, input_ids3: torch.LongTensor, **kwargs) -> Dict[str, Any]:
        """
        Implement in subclasses of :class:`~transformers.PreTrainedModel` for custom behavior to prepare inputs in the
        generate method.
        """
        # return {"input_ids": input_ids, "input_ids2": input_ids2, "input_ids3": input_ids3,'attention_mask':kwargs['attention_mask'],
        #     'attention_mask2':kwargs['attention_mask2'], 'attention_mask3':kwargs['attention_mask3'],}
        return {"input_ids": input_ids, "input_ids2": input_ids2, "input_ids3": input_ids3,}
    ...
    def _prepare_encoder_decoder_kwargs_for_generation(
        self, input_ids: torch.LongTensor,input_ids2: torch.LongTensor,input_ids3: torch.LongTensor, model_kwargs
    ) -> Dict[str, Any]:
        # print(model_kwargs)
        ...
        encoder_outputs1=encoders[0](input_ids, return_dict=True, attention_mask=encoder_kwargs['attention_mask'],enc='one')
        encoder_outputs2=encoders[1](input_ids2, return_dict=True, attention_mask=encoder_kwargs['attention_mask2'],enc='two')
        encoder_outputs3=encoders[2](input_ids3, return_dict=True, attention_mask=encoder_kwargs['attention_mask3'],enc='three')
        # encoder=BaseModelOutput(
        #     last_hidden_state=encoder_outputs1[0]+encoder_outputs2[0]+encoder_outputs3[0],
        ...
        # encoder=(encoder_outputs1[0]+encoder_outputs2[0]+encoder_outputs3[0],encoder_outputs1[1]+encoder_outputs2[1]+encoder_outputs3[1],encoder_outputs1[2]+encoder_outputs2[2]+encoder_outputs3[2])
        model_kwargs["encoder_outputs1"]: ModelOutput = encoder_outputs1#(input_ids, return_dict=True, **encoder_kwargs)
        model_kwargs["encoder_outputs2"]: ModelOutput = encoder_outputs2#(input_ids, return_dict=True, **encoder_kwargs)
        model_kwargs["encoder_outputs3"]: ModelOutput = encoder_outputs3#(input_ids, return_dict=True, **encoder_kwargs)
        return model_kwargs
    ...
    def _expand_inputs_for_generation(
        ...
        encoder_outputs1: ModelOutput = None,
        encoder_outputs2: ModelOutput = None,
        encoder_outputs3: ModelOutput = None,
        **model_kwargs,
        ...
        if is_encoder_decoder:
            assert encoder_outputs1 is not None
            encoder_outputs1["last_hidden_state"] = encoder_outputs1.last_hidden_state.index_select(
                0, expanded_return_idx.to(encoder_outputs1.last_hidden_state.device)
            )
            encoder_outputs2["last_hidden_state"] = encoder_outputs2.last_hidden_state.index_select(
                0, expanded_return_idx.to(encoder_outputs2.last_hidden_state.device)
            )
            encoder_outputs3["last_hidden_state"] = encoder_outputs3.last_hidden_state.index_select(
                0, expanded_return_idx.to(encoder_outputs3.last_hidden_state.device)
            )
            model_kwargs["encoder_outputs1"] = encoder_outputs1
            model_kwargs["encoder_outputs2"] = encoder_outputs2
            model_kwargs["encoder_outputs3"] = encoder_outputs3
    ...
    def _update_model_kwargs_for_generation(
        ...
        if not is_encoder_decoder:
            ...
            if "attention_mask2" in model_kwargs:
                attention_mask2 = model_kwargs["attention_mask2"]
                model_kwargs["attention_mask2"] = torch.cat(
                    [attention_mask2, attention_mask2.new_ones((attention_mask2.shape[0], 1))], dim=-1
                )
            if "attention_mask3" in model_kwargs:
                attention_mask3 = model_kwargs["attention_mask3"]
                model_kwargs["attention_mask3"] = torch.cat(
                    [attention_mask3, attention_mask3.new_ones((attention_mask3.shape[0], 1))], dim=-1
                )
    ...
    def generate(
        ...
        **model_kwargs,
        ...
        if self.config.is_encoder_decoder:
            ...
            model_kwargs = self._prepare_encoder_decoder_kwargs_for_generation(input_ids,input_ids2,input_ids3, model_kwargs)
            ...
            if "decoder_input_ids" in model_kwargs:
                ...
                input_ids2 = self._prepare_decoder_input_ids_for_generation(
                    input_ids2, decoder_start_token_id=decoder_start_token_id, bos_token_id=bos_token_id
                )#goes in else
                input_ids3 = self._prepare_decoder_input_ids_for_generation(
                    input_ids3, decoder_start_token_id=decoder_start_token_id, bos_token_id=bos_token_id
                )#goes in else

            if "encoder_outputs1" not in model_kwargs or not isinstance(model_kwargs["encoder_outputs1"], ModelOutput):
    ...
    def beam_search(
        ...
        **model_kwargs,
        ...
        # model_kwargs['encoder_outputs2']['attention_mask2']=model_kwargs['encoder_outputs2']['attention_mask']
        # del thisdict["model"]
        ...
        while cur_len < max_length:
            # print(*model_kwargs,'model_inputs in generation_utils')
            # sys.exit(0)


            model_inputs = self.prepare_inputs_for_generation(input_ids,input_ids2,input_ids3, **model_kwargs)
