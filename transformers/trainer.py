...
class Trainer:
    ...
    def train(
        ...
        **kwargs,
        ...
        print(resume_from_checkpoint,'resume_from_checkpointttttttttttttttttttttttttttttttt',model_reloaded)
        if resume_from_checkpoint is not None and os.path.isfile(os.path.join(resume_from_checkpoint, WEIGHTS_NAME)):
            logger.info(f"Loading model from {resume_from_checkpoint}).")
            if isinstance(self.model, PreTrainedModel):
                self.model = self.model.from_pretrained(resume_from_checkpoint)
                print(resume_from_checkpoint,'resume_from_checkpointttttttttttttttttttttttttttttttt',model_reloaded)
                # self.model=self.model.init_weights()
                model_reloaded = True
                # print('initialized with model_init_weights()')
            else:
        ...
        #     break
        # sys.exit(0)
        # print(self.data_collator,'self.data_collator')
        ...
        # j=0
        # for i in train_dataloader:
        #     print(i['attention_mask'].shape,i['attention_mask2'].shape,i['attention_mask3'].shape)
        #     j+=1
        #     if(j==5):
        #         break
        # sys.exit(0)
    ...
    def _prepare_inputs(self, inputs: Dict[str, Union[torch.Tensor, Any]],features_ext) -> Dict[str, Union[torch.Tensor, Any]]:
        """
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        ...
        model.train()
        feat_path='peer_data/feat_decoder/feat_all.pth'
        features_ext=torch.load(feat_path)
        inputs = self._prepare_inputs(inputs,features_ext)
        # print(inputs,'inputs')
        # sys.exit(0)
    ...
    def prediction_loop(
        ...
        df=pd.DataFrame(columns=['index','decision'])
        for step, inputs in enumerate(dataloader):
            # print(inputs,'inputs')
            loss, logits, labels,deci = self.prediction_step(model, inputs, prediction_loss_only, ignore_keys=ignore_keys)
            df=df.append({'index':step,'decision':deci},ignore_index=True)
            # print(logits)
            # print(label)
            if loss is not None:
        ...
        df.to_csv('decision_label.csv',index=False)
        if self.args.past_index and hasattr(self, "_past"):
